...
class NP_Calendar extends NucleusPlugin
{
	...
	function _drawCalendar($timestamp, &$blog, $linkall, $category, $useextlib)
	{
		...
		if ($last_month < $oldestdate[1]
		 && $year == $oldestdate[0])          $past = false;
		else
		{
			if ($last_month > $oldestdate[1]
			 && $last_year < $oldestdate[0])  $past = false;
			else                              $past = true;
		}
		...
		if ($startsun == 'yes')
		{
			...
			{
				if(in_array($mday, $special_day)) $class = 'special';
				elseif ($mday == $to_day
				     && $this_month == $month
